...
def initialization(params):
    ...
    if False:
        for k in range(params['geometry']['i']):
            positions[k] = (0.1*positions[k][0],0.1*positions[k][1])
    return home, work
...
class Disk():
    ...
    def callback(self, day, day_fraction, status, positions, home, work, plt, **ignore_other_kwargs):
        ...
        if day_fraction==0:
            ...
            self.time_history.append(day + day_fraction)
            self.axs[1][0].clear()
            plot_points(plt=self.axs[1][0], positions=vulnerable_positions, status=vulnerable_status, title='Vulnerable')
            self.axs[1][0].figure
